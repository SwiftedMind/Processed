...
public struct SharedProcess<Process>: Equatable, DynamicProperty where Process: Equatable, Process: Sendable {

    public enum ProcessState: Equatable {
        case idle
        case running(Process)
        case failed(process: Process, error: Swift.Error)
    ...
    public static func unique(initialState: ProcessState = .idle) -> SharedProcess<UniqueProcess> where Process == UniqueProcess {
        .init(initialState: initialState)
    }

    public static func shared(by type: Process.Type, initialState: ProcessState = .idle) -> SharedProcess<Process> {
        SharedProcess<Process>(initialState: initialState)
    }
    ...
    public func start(_ process: Process) {
        cancel(resetToIdle: false)
        state = .running(process)
    }

    public func start() where Process == UniqueProcess {
        start(.init())
    }
    ...
    public func setFailed(_ process: Process, error: Swift.Error) {
        cancel(resetToIdle: false)
        state = .failed(process: process, error: error)
    }

    public func setFinished(_ process: Process) {
        state = .finished(process)
    }
    ...
    public func isRunning(_ process: Process) -> Bool {
    ...
    public static func == (lhs: SharedProcess<Process>, rhs: SharedProcess<Process>) -> Bool {
        lhs.state == rhs.state
    }
}
